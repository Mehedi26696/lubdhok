    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="text-center mb-20 opacity-0 animate-fade-in-up">
          ...
          <h1 className="text-5xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-white via-blue-100 to-violet-100 bg-clip-text text-transparent leading-tight">
            Lubdhok-29
          </h1>
          ...
          {/* Hero Image */}
          <div className="relative w-full h-96 md:h-[500px] rounded-xl overflow-hidden shadow-2xl mx-auto max-w-5xl border border-slate-600/50">
            <Image
              src="/batch.jpg"
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-slate-900/40 to-transparent flex items-end">
              <div className="p-8 text-white">
                ...
                <p className="text-lg md:text-xl text-blue-200">3rd Year Students - Lubdhok-29</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-20">
          {stats.map((stat, index) => (
            <div 
              key={index}
              className="bg-slate-800/60 backdrop-blur-md rounded-xl p-8 text-center border border-slate-600/50 hover:border-blue-400/50 hover:-translate-y-1 transition-all duration-300 group opacity-0 animate-fade-in-up"
        ...
        <div className="mb-20">
          <div className="text-center mb-12 opacity-0 animate-fade-in-up" style={{animationDelay: '0.6s', animationFillMode: 'forwards'}}>
            <h2 className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-white via-blue-100 to-violet-100 bg-clip-text text-transparent">
              What We Value
            </h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((value, index) => (
              <div 
                key={index}
                className="bg-slate-800/60 backdrop-blur-md rounded-xl p-6 text-center border border-slate-600/50 hover:border-blue-400/50 hover:-translate-y-1 transition-all duration-300 group opacity-0 animate-fade-in-up"
        ...
        <div className="mb-20">
          ...
          <div className="relative">
            {/* Timeline Line */}
            <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-gradient-to-b from-blue-500 to-violet-500 rounded-full"></div>
            
            {timeline.map((item, index) => (
              <div 
                key={index}
                className={`relative flex items-center mb-12 opacity-0 animate-fade-in-up ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}
                style={{ animationDelay: `${1.2 + (index * 0.2)}s`, animationFillMode: 'forwards' }}
              >
                <div className={`w-1/2 ${index % 2 === 0 ? 'pr-12 text-right' : 'pl-12 text-left'}`}>
                  <div className="bg-slate-800/60 backdrop-blur-md rounded-xl p-6 border border-slate-600/50 hover:border-blue-400/50 transition-all duration-300 group">
                    <div className="text-xl font-bold text-blue-400 mb-2 group-hover:text-blue-300 transition-colors duration-300">{item.year}</div>
                ...
                {/* Timeline Dot */}
                <div className="absolute left-1/2 transform -translate-x-1/2 w-4 h-4 bg-gradient-to-r from-blue-500 to-violet-500 rounded-full border-4 border-slate-800 shadow-lg z-10"></div>
              </div>
            ))}
          </div>
        </div>
